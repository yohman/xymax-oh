...
function initMap(geoJsonData) {
	...
	map.fitBounds(bounds, { padding: Math.max(100, xymax.defaults.padding), pitch: xymax.defaults.pitch });
		...
		window.updateSidePanelMultiple_OLD = function updateSidePanelMultiple(featuresMap) {
			...
			const startYear = firstFeature.properties.startYear || '';
				// Expose map object globally for dropdown event
				window.map = map;
		...
		// Click event for polygon selection (multi-select with updateSidePanelSingle)
		map.on('click', 'tokyo-layer', (e) => {
			...
			if (selectedKeyCodes.length > 0) {
				map.setFilter('tokyo-layer-highlight', ['in', 'KEY_CODE', ...selectedKeyCodes]);
				// Always use updateSidePanelSingle with all selected keycodes
				if (window.updateSidePanelSingle) {
					window.updateSidePanelSingle(selectedKeyCodes);
				}
				isSticky = true;
				...
				if (map.getSource('area-outline')) {
					map.removeSource('area-outline');
				}
				isSticky = false;
		...
		map.on('click', (e) => {
			...
			if (features.length === 0) {
				// Clicked on empty area - clear all selections and unstick
				...
				// Remove highlight
				map.setFilter('tokyo-layer-highlight', ['==', 'KEY_CODE', '']);
				// Remove area outline if present
				if (map.getLayer('area-outline')) {
					map.removeLayer('area-outline');
				}
				if (map.getSource('area-outline')) {
				...
				sidePanel.classList.remove('open', 'sticky');
				document.querySelector('.side-panel-header').classList.remove('sticky');
				// Resume hover: clear lastHoveredKeyCode so hover can update
				window.lastHoveredKeyCode = undefined;
				// Reset area dropdown to default value
		...
		// Add hover event for info panel updates when not sticky
		if (map) {
			map.on('mousemove', 'tokyo-layer', function(e) {
				// Disable hover info panel updates when sticky (selection) mode is active
				if (window.isSticky) return;
				if (!window.selectedFeatures || window.selectedFeatures.size === 0) {
					if (e.features && e.features.length > 0) {
						const feature = e.features[0];
						// Only update if KEY_CODE is different
						if (window.lastHoveredKeyCode !== feature.properties.KEY_CODE) {
							window.lastHoveredKeyCode = feature.properties.KEY_CODE;
							// Highlight hover polygon
							map.setFilter('tokyo-layer-hover', ['==', 'KEY_CODE', feature.properties.KEY_CODE]);
							// Show info for hovered polygon
							if (window.updateSidePanelSingle) {
								const keycodes = [feature.properties.KEY_CODE];
								window.updateSidePanelSingle(keycodes);
							}
			...
			map.on('mouseleave', 'tokyo-layer', function() {
				window.lastHoveredKeyCode = undefined;
				// Disable hover info panel updates when sticky (selection) mode is active
				if (window.isSticky) return;
				if (!window.selectedFeatures || window.selectedFeatures.size === 0) {
					...
					map.setFilter('tokyo-layer-hover', ['==', 'KEY_CODE', '']);
					// Optionally clear info panel or keep last hovered
				}
			});
		}
	});
